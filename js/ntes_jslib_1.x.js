(function(R, f) {
	...
	var s = {
			...
			if(_ && _.id !== $ && A.all) {
				...
				if(_) {
					...
					for(var B = 0; B < _.length; B++)
						if(this.getAttribute(_[B], "id") === $) return _[B]
				}
			} else return _
		},
		...
		getElemsByAttribute: function(B, D) {
			var _ = [],
				...
				C = "~=" === D[1] ? " " + D[2] + " " : D[2];
			if(A)
				while($ = B[E++]) A(this.getAttribute($, D[0]), C) && _.push($);
			return _
		},
		operators: {
			"#": function(_, $) {
	...
	T.util = {
		...
		each: function(A, _, $) {
			...
			if($) {
				if(C) {
					for(D in A)
						if(false === _.apply(A[D], $)) break
				} else
					while(++D < B)
						if(false === _.apply(A[D], $)) break
			} else if(C) {
				for(D in A)
					if(false === _.call(A[D], D, A[D])) break
			} else
				while(++D < B)
					if(false === _.call(A[D], D, A[D])) break;
			return A
	...
	T.dom = {
		wrapByArray: function($) {
			if($)
				if($.nodeType !== f || $.setInterval) return [$];
	...
	function t(A, _) {
		if("" === this.style.cssText && "string" === typeof _) this.style.cssText = _;
			...
			for(var $ in A) this.style[$] !== f && (this.style[$] = A[$])
	}
	...
	T.style = {
		...
		hasClass: function(_, $) {
			...
			if(A > 0) {
				$ = " " + $ + " ";
				while(--A >= 0)
					if((" " + _[A].className + " ").indexOf($) >= 0) return true
			}
			return false
		},
		parse: function(C) {
		...
		send: function(G, C, B, H, A) {
			...
			if(H.headers)
				for(var F in H.headers)
					if("content-type" === F.toLowerCase()) D.push(H.headers[F]);
